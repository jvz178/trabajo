    ...
    value="{{ isset($student->firstname)?$student->firstname:old('firstname')}}">
    {!! $errors->first('firstname','<div class="invalid-feedback">:message</div>')!!}
    
    </div>
    
    <input type="submit" class="btn btn-success" value="{{$Modo=='create' ? 'Crear alumno':'Modificar alumno'}}">
    <a class="btn btn-primary" href="{{url('student')}}">Volver</a>